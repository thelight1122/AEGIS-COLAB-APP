...
def detect_wisdom(response, exp_items, found_items, shelf_items, archive_items, query):
    ...
    PCT (Persistent Content Tensor / working context) is the Known side.
    ...
    Path 2 (Pondered):
      ...
      PCT must resonate OR synthesis rate must be high enough to constitute a new Known.
    ...
    # C_pct — Known side: PCT (working context) specifically
    # This is what makes an Experienced Known distinct from mere pondering.
    pct_items = [e for e in found_items if e.get("tensor") == "PCT"]
    pct_toks = set()
    for e in pct_items:
        pct_toks |= e.get("toks", set())
    for e in archive_items:
        if e.get("tensor") == "PCT" or e.get("source") == "ARCHIVE":
            pct_toks |= e.get("toks", set())
    for e in shelf_items:
        pct_toks |= toks(e.get("text", ""))

    # C_other — NCT and SPINE: received foundation that drives pondering
    # Contributes to Path 2 synthesis but does not constitute the Known side alone.
    ...
    a_resonance   = len(r_toks & a_toks)    # Felt (PEER) tokens in response
    pct_resonance = len(r_toks & pct_toks)  # Known (PCT) tokens in response
    other_resonance = len(r_toks & other_toks)  # Foundation pondering tokens in response

    # CO = C_pct / A  — PCT organizing PEER (floor of 1 on A)
    co = pct_resonance / max(1, a_resonance)

    # Path 1: PEER (Felt) + PCT (Known) both resonate in response, CO >= 1
    path1 = (
        a_resonance >= 2
        and pct_resonance >= 2
        and co >= 1.0
    ...
    # Path 2: Foundation pondered (NCT/SPINE/PCT), no prior PEER experience
    # The pondering IS the Felt side — response must synthesize beyond echo
    path2 = False
    if not exp_items and found_items:
        ...
        path2 = (
            (pct_resonance >= 2 or other_resonance >= 3)  # Foundation integrated
            and synthesis_rate >= 0.25                     # 25%+ genuine synthesis
        )
        if path2:
